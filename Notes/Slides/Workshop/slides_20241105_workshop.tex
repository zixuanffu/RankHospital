...
% \begin{frame}
%   \frametitle{Hammer: Methods}

%   \begin{itemize}\itemsep=12pt
%     \item Compound decision (compound loss/risk) proposed by
%           \citet{robbins1956empirical}.
%     \item Compound risk $\sim$ Bayesian risk: $\int \int L(\theta_i,
%             \delta(\hat{\theta}_i))dP(\hat{\theta}_i|\theta_i)dG(\theta)$
%     \item Empirically estimate the prior distribution of $\theta_i$ using nonparametric
%           MLE. \citep{kiefer1956consistency, koenker2014convex, gu2017empirical}
%     \item Selection as a compound problem \citet{gu2023invidious}.
%   \end{itemize}

% \end{frame}

% \begin{frame}{Nail: Data}

%   \begin{itemize}\itemsep=12pt
%     \item French hospital data on input, output, ownership. \citep{croiset2024hospitals}
%     \item Panel data estimation of production function/factor demand
%           function.\cite{arellano1991some, arellano1995another,blundell1998initial}
%     \item Noisy fixed effect. \citep{chetty2014measuring,kline2022systemic}
%   \end{itemize}
...
\begin{frame}[label=observation]
  ...
  \end{align*}
  In our factor demand function specification, we have $Y_{it}=\log(x_{it})-\beta\log(y_{it})$. \hyperlink{normality}{\beamergotobutton{Appendix}}
\end{frame}
...
\begin{frame}[label=limitation]{Limitation}
  \begin{itemize}\itemsep=12pt
    ...
    \item Specification, endogeneity, normality assumption on $\varepsilon_{it}$,
          ...
          \hyperlink{normality}{\beamergotobutton{Next}} \end{itemize}
\end{frame}
