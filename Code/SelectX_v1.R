...
level_plot <- function(Z, alpha = 0.04, gamma = 0.2, tail = "R", cindex = c(2, 5), constraint = "cap",
    ...
    Rules <- c("TPKW", "TPKWs", "PMKW", "PMKWs", "MLE", "Z-score", "E&M", "JS")
    # estmean = mean(Z$S)
    # estvar = uniroot(psi, c(0.001, 4), s = (Z$S-estmean)^2, d = 1/Z$W, extendInt = "yes")$root
    est <- optim(c(0, 1), lik, x = Z$S, s = sqrt(1 / Z$W))$par
    estmean <- est[1]
    estvar <- est[2]
    pmr <- predict(Z$f, Z$S, newsigma = sqrt(1 / Z$W))
    pm <- predict(Z$fs, Z$S, newsigma = sqrt(1 / Z$W))
    em <- EMrule(Z$S, sqrt(1 / Z$W), mean = estmean, estvar = estvar)
    linear <- estmean + (Z$S - estmean) * estvar / (estvar + (1 / Z$W))
    ...
    if (7 %in% cindex) { # more costly to do EM level curve
        for (i in 1:length(ygrid)) {
            for (j in 1:length(wgrid)) {
                cls[i, j, 7] <- EMrule(c(ygrid[i], Z$S), c(sqrt(1 / wgrid[j]), sqrt(1 / Z$W)), mean = estmean, estvar = estvar)$v[1]
            }
            print(i)
        }
    }
    if (tail == "R") {
        ...
        tpr <- Lfdr.GLmix_temp(Z$S, Z$f, sqrt(1 / Z$W), cnull = cnullr)
        tp <- Lfdr.GLmix_temp(Z$S, Z$fs, sqrt(1 / Z$W), cnull = cnull)
        ...
        tem1 <- try(Finv(gamma, ThreshFDREM, mean = estmean, estvar = estvar, Bhat = em$Bhat, s = sqrt(1 / Z$W), alpha = alpha, tail = "R"), silent = TRUE)
        tlm0 <- quantile(linear, 1 - alpha)
        tlm1 <- try(Finv(gamma, ThreshFDREM, mean = estmean, estvar = estvar, Bhat = 1 / Z$W / (estvar + (1 / Z$W)), s = sqrt(1 / Z$W), alpha = alpha, tail = "R"), silent = TRUE)
        ...
        if ("cap" %in% constraint) {
            if (cindex[1] != 5) {
                contour(ygrid, log(wgrid), cls[, , cindex[[1]]],
                    lwd = 2, col = 4,
            ...
            if (cindex[2] != 5) {
                contour(ygrid, log(wgrid), cls[, , cindex[[2]]],
                    lwd = 2,
    ...
    if (tail == "L") {
        ...
        tpr <- Lfdr.GLmix_temp(Z$S, Z$f, sqrt(1 / Z$W), cnull = cnullr, tail = "L")
        ...
        tem1 <- try(Finv(gamma, ThreshFDREM, mean = estmean, estvar = estvar, Bhat = em$Bhat, s = sqrt(1 / Z$W), alpha = alpha, tail = "L"), silent = TRUE) # control FDR by plugging Gaussian G estimates
        tlm0 <- quantile(linear, alpha)
        tlm1 <- try(Finv(gamma, ThreshFDREM, mean = estmean, estvar = estvar, Bhat = 1 / Z$W / (estvar + (1 / Z$W)), s = sqrt(1 / Z$W), alpha = alpha, tail = "L"), silent = TRUE) # control FDR by plugging Gaussian G estimates
...
# level_plot<-function(Z,alpha=0.22,gamma=0.2, tail = "R", cindex=c(2,5),constraint = "cap",ygrid=seq(1,1.6,length),wgrid=8*(1:100)){
#     cols <- c("grey","blue","red")
#     Rules <- c("TPKW","TP","PMKW","PMKWs","MLE","Z-score","E&M","JS")
#     est <- optim(c(0,1),lik,x=Z$S,s=sqrt(Z$s))$par
#     estmean <- est[1]
#     estvar <- est[2]
#     # ---- x axis is the $\hat{\theta}_i$  (Z$S) ---- #
#     # ---- y axis is the (transformed) variance $\sigma_i^2$ (no hat) (Z$W)---- #
#     pmr <- predict(Z$f,Z$S,newsigma=sqrt(var_func(Z$W)))
#     pm <- predict(Z$fs,Z$S,newsigma=sqrt(var_func(Z$W)))
#     em <- EMrule(Z$S,sqrt(var_func(Z$W)),mean=estmean,estvar=estvar)
#     linear <- estmean + (Z$S - estmean)*estvar/(estvar + var_func(Z$W))
#     cls <- array(NA,c(length(ygrid),length(wgrid),length(Rules)))
#     if(7 %in% cindex){ # more costly to do EM level curve
#         for(i in 1:length(ygrid)){
#             for(j in 1:length(wgrid)){
#                 cls[i,j,7] <- EMrule(c(ygrid[i],Z$S), c(sqrt(var_func(wgrid[j])),sqrt(var_func(Z$W))),mean=estmean,estvar=estvar)$v[1]
#             }
#             # print(i)
#         }
#     }
#     if (tail == "R"){
#         sR <- selectR1d(Z,alpha=alpha,gamma=gamma)
#         zv <- ((Z$S-1)*sqrt(1/var_func(Z$W)))
#         cnullr <- qKW(Z$f,1-alpha)
#         cnull <- qKW(Z$fs,1-alpha)
#         tpr <- Lfdr.GLmix_temp(Z$S,Z$f,sqrt(var_func(Z$W)),cnull=cnullr,tail = tail)
#         tp <- Lfdr.GLmix_temp(Z$S,Z$fs,sqrt(var_func(Z$W)),cnull=cnull, tail=tail)
#         ttpr0 <- quantile(tpr,1-alpha)
#         ttpr1 <- try(Finv(gamma,ThreshFDR,interval=c(0.1,0.9),stat=tpr,v=tpr),silent=TRUE)
#         ttp0 <- quantile(tp,1-alpha)
#         ttp1 <- try(Finv(gamma,ThreshFDR,interval=c(0.1,0.9),stat=tp,v=tp),silent=TRUE)
...
#         tzv1 <- try(Finv(gamma,ThreshFDR,stat=zv,v=tp),silent=TRUE)
#         tem0 <- quantile(em$v,1-alpha)
#         tem1 <- try(Finv(gamma,ThreshFDREM,mean=estmean,estvar=estvar,Bhat=em$Bhat,s=sqrt(1/var_func(Z$W)),alpha=alpha,tail="R"),silent=TRUE)
#         tlm0 <- quantile(linear,1-alpha)
#         tlm1 <- try(Finv(gamma,ThreshFDREM,mean=estmean,estvar=estvar,Bhat=1/var_func(Z$W)/(estvar + 1/var_func(Z$W)),s=sqrt(1/var_func(Z$W)),alpha=alpha,tail="R"),silent=TRUE)
#         # ---- a matrix to store the threshold for each rule (1st column for cap only, 2nd column for cap and fdr) ---- #
#         thresh <- matrix(NA,length(Rules),2)
#         thresh[1, ] <- c(ttpr0, max(ttpr0, ttpr1))
#         thresh[2, ] <- c(ttp0, max(ttp0, ttp1))
#         thresh[3, ] <- c(tpmr0, max(tpmr0, tpmr1))
...
#         for (i in 1:length(ygrid)) {
#             for (j in 1:length(wgrid)) {
#                 cls[i, j, 1] <- Lfdr.GLmix_temp(ygrid[i], Z$f, sqrt(var_func(wgrid[j])), cnull = cnullr)
#                 cls[i, j, 2] <- Lfdr.GLmix_temp(ygrid[i], Z$fs, sqrt(var_func(wgrid[j])), cnull = cnull)
#                 cls[i, j, 3] <- predict(Z$f, ygrid[i], newsigma = sqrt(var_func(wgrid[j])))
#                 cls[i, j, 4] <- predict(Z$fs, ygrid[i], newsigma = sqrt(var_func(wgrid[j])))
#                 cls[i, j, 5] <- ygrid[i]
#                 cls[i, j, 6] <- ((ygrid[i] - 1) * sqrt(var_func(wgrid[j])))
#                 cls[i, j, 8] <- estmean + (ygrid[i] - estmean) * estvar / (estvar + var_func(wgrid[j]))
#     }
# }
#  # ---- classify the dots into 3 categories: agree, extra TP, extra MLE ---- #
#     A <- sR$A # cap and fdr constraint
#     B <- sR$B # cap constraint
#     Bagree <- intersect(which(B[,cindex[1]] == 1),which(B[,cindex[2]] == 1))
#     Bdis1 <- setdiff(which(B[,cindex[1]] == 1),which(B[,cindex[2]] == 1))
...
#     Adis1 <- setdiff(which(A[,cindex[1]] == 1),which(A[,cindex[2]] == 1))
#     Adis2 <- setdiff(which(A[,cindex[2]] == 1),which(A[,cindex[1]] == 1))
# # ---- 2 plots (cindex[1] and cindex[2] respectively ---- #
#     if (constraint == "cap") {
#         if (cindex[1] != 5) {
#             contour(ygrid, wgrid, cls[, , cindex[1]],
#                 lwd = 2, col = 4,
#                 levels = round(thresh[cindex[1], 1], digits = 3),
#                 xlab = expression(hat(theta)[i]), ylab = expression(f(sigma[i]^2)), drawlabels = FALSE
#             )
#         } else {
#             abline(v = thresh[cindex[1], 1], lwd = 2, col = 2)
#         }
...
#             )
#         } else {
#             abline(v = thresh[cindex[2], 1], lwd = 2)
#         }
#         points (Z$S[Bagree],Z$W[Bagree],col="grey",cex=0.5)
#         points (Z$S[Bdis1],Z$W[Bdis1],col="blue",cex=0.5)
#         points (Z$S[Bdis2],Z$W[Bdis2],col="red",cex=0.5)
#         text <- c(
#                 "Agree", paste(Rules[cindex[1]], " extra", sep = ""),
#                 paste(Rules[cindex[2]], " extra", sep = "")
#             )
...
# if (tail == "L") {
#         sL <- selectL1d(Z, alpha = alpha, gamma = gamma)
#         zv <- ((1 - Z$S) * sqrt(1/var_func(Z$W)))
#         cnullr <- qKW(Z$f, alpha)
#         cnull <- qKW(Z$fs, alpha)
#         tpr <- Lfdr.GLmix_temp(Z$S, Z$f, sqrt(var_func(Z$W)), cnull = cnullr, tail = "L")
#         tp <- Lfdr.GLmix_temp(Z$S, Z$fs, sqrt(var_func(Z$W)), cnull = cnull, tail = "L")
#         ttpr0 <- quantile(tpr, 1 - alpha)
#         ttpr1 <- try(Finv(gamma, ThreshFDR, interval = c(0.1, 0.9), stat = tpr, v = tpr), silent = TRUE)
#         ttp0 <- quantile(tp, 1 - alpha)
#         ttp1 <- try(Finv(gamma, ThreshFDR, interval = c(0.1, 0.9), stat = tp, v = tp), silent = TRUE)
...
#         for (i in 1:length(ygrid)) {
#             for (j in 1:length(wgrid)) {
#                 cls[i, j, 1] <- Lfdr.GLmix_temp(ygrid[i], Z$f, sqrt(var_func(wgrid[j])), cnull = cnullr, tail = "L")
#                 cls[i, j, 2] <- Lfdr.GLmix_temp(ygrid[i], Z$fs, sqrt(var_func(wgrid[j])), cnull = cnull, tail = "L")
#                 cls[i, j, 3] <- predict(Z$f, ygrid[i], newsigma = sqrt(var_func(wgrid[j])))
#                 cls[i, j, 4] <- predict(Z$fs, ygrid[i], newsigma = sqrt(var_func(wgrid[j])))
#                 cls[i, j, 5] <- ygrid[i]
#                 cls[i, j, 6] <- ((1 - ygrid[i]) * sqrt(var_func(wgrid[j])))
#                 cls[i, j, 8] <- estmean + (ygrid[i] - estmean) * estvar / (estvar + var_func(wgrid[j]))
#             }
#         }
#         A <- sL$A # cap and fdr constraint
...
#                     lwd = 2,
#                     levels = round(thresh[cindex[2], 1], digits = 3),
#                     xlab = expression(hat(theta)[i]), ylab = expression(f(sigma[i]^2)), add = TRUE, col = 2, drawlabels = FALSE
#                 )
#             } else {
#                 abline(v = thresh[cindex[2], 1], lwd = 2, col = 2)
#             }
