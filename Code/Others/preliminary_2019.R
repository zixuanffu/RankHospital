...
capact19 <- fread("Data/In/sae_stat/SAE 2019/CAPACT_PM_PNM/Bases CSV/capact19.csv")

# Select the columns of interest (ID and status)
id_stj <- capact19[, .(AN, FI, RS, FI_EJ, STJ, STJR)]
# annee,
# N° Finess
...
id_stj <- unique(id_stj)
# 3913 unique observations

# Relabel CHU
id_stj[grepl("CHU", RS) & STJR == 1, STJR := 0]

#### 2. Hospital input and output ####
# Load the main dataset
data2019r <- fread("Data/In/sae_stat/SAE 2019/Bases statistiques/Bases CSV/SYGEN_2019r.csv")

# Check if FI, FI_EJ, RS are in the columns
print("FI" %in% colnames(data2019r)) # 1
print("FI_EJ" %in% colnames(data2019r)) # 1
print("RS" %in% colnames(data2019r)) # 0
...
cols <- c("FI", "FI_EJ", labor)
data2019r_input <- data2019r[, ..cols]
data2019r_input[is.na(data2019r_input)] <- 0

# remove those that do not have any labor input reported
data2019r_input[, input_sum := rowSums(.SD, na.rm = TRUE), .SDcols = labor]
data2019r_input <- data2019r_input[input_sum != 0]


# Merge the data
data2019r_input_id <- merge(id_stj, data2019r_input, by.x = c("FI", "FI_EJ"), by.y = c("FI", "FI_EJ"), all.y = TRUE)

data2019r_input_id <- data2019r_input_id[!is.na(STJ)]



#### 3. Summarize ####
dt <- copy(data2019r_input_id)
setDT(dt)
dt[, `:=`(MD = ETPSAL_SPEMED + ETPSAL_SPECHI, IF = ETP_INFAVECSPE + ETP_INFSANSSPE, AS = ETP_AID, NM = ETP_TOT_HORS_SOINS)]
input_sum <- dt[, lapply(.SD, function(x) sum(x, na.rm = TRUE)), by = STJR, .SDcols = c("MD", "IF", "AS", "NM")]
input_mean <- dt[, lapply(.SD, function(x) mean(x, na.rm = TRUE)), by = STJR, .SDcols = c("MD", "IF", "AS", "NM")]
...
#### 1. Hospital ID and status ####
# Load the data for identifying hostpital
id <- fread("Data/In/sae_admin/SAE 2019/Base CSV/ID_2019.csv")
...
#### 2. Hospital input and output ####
# Load the main dataset
dt <- fread("Data/In/sae_admin/SAE 2019/Base CSV/SYGEN_2019.csv")
