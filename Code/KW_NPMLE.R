...
par(mfrow = c(1, 2))
pdf("Figures/2013-2022/GMM/WTLVmix_u.pdf", width = 8, height = 5)
plot(f_t$u, f_t$fu,
    main = expression(paste("Density of ", theta, sep = "")),
    xlab = expression(theta), ylab = expression(f(theta)), type = "l"
)
dev.off()
pdf("Figures/2013-2022/GMM/WTLVmix_v.pdf", width = 8, height = 5)
...
g$fuv <- f_g$fuv
pdf("Figures/2013-2022/GMM/WGLVmix.pdf", width = 8, height = 5)
...
g$fuv <- fg$fuv
pdf("Figures/2013-2022/GMM/GLVmix.pdf", width = 8, height = 5)
pl <- cloud(fuv ~ theta * sigma,
    data = g, type = "h", lwd = 2,
    zlim = c(0, max(g$fuv)), scales = list(
        arrows = FALSE,
